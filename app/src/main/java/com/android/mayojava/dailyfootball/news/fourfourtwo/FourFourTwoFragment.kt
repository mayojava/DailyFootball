...
class FourFourTwoFragment: BaseFragment() {
    @Inject lateinit var viewModelFactory: AppViewModelFactory

    private val adapter = RecyclerAdapter(rowLayoutId = R.layout.news_row_item) {
        val direction = NewsHostFragmentDirections.toNewsDetails(it.id.toInt(), Constants.FOURFOURTWO)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val viewModel = ViewModelProviders.of(this, viewModelFactory).get(FourFourTwoViewModel::class.java)
        viewModel.fourFourTwoNews.observe(viewLifecycleOwner, Observer {
            adapter.setItems(it)
        })
    }
}
